...
struct LSH512_Context
{
	...
	unsigned long long cv_l[8];
	unsigned long long cv_r[8];
	uint8_t last_block[256];
};

struct LSH512_internal
{
	unsigned long long submsg_e_l[16 / 2];	/* even left sub-message*/
	unsigned long long submsg_e_r[16 / 2];	/* even right sub-message*/
	unsigned long long submsg_o_l[16 / 2];	/* odd left sub-message*/
	unsigned long long submsg_o_r[16 / 2];	/* odd right sub-message*/
};



static const unsigned long long g_IV224[16] = 
{
...
static const unsigned long long g_IV256[16] = 
{
...
static const unsigned long long g_IV384[16] = 
{
...
static const unsigned long long g_IV512[16] = 
{
...
static const uint32_t g_gamma512[8] = { 0, 16, 32, 48, 8, 24, 40, 56 };

static const unsigned long long g_StepConstants[8* 28] = 
{
 ...
 static inline void load_msg_blk(struct LSH512_internal* i_state, const unsigned long long* msgblk)
{
...
static inline void load_sc(const unsigned long long** p_const_v, uint32_t i)
{
	*p_const_v = &g_StepConstants[i];
}

static inline void msg_add_even(unsigned long long cv_l[8], unsigned long long cv_r[8], struct LSH512_internal* i_state)
{
...
static inline void msg_add_odd(unsigned long long cv_l[8], unsigned long long cv_r[8], struct LSH512_internal* i_state)
{
...
static inline void  add_blk(unsigned long long cv_l[8], unsigned long long cv_r[8])
{
	cv_l[0] += cv_r[0];
	cv_l[1] += cv_r[1];
	cv_l[2] += cv_r[2];
...
static inline void  rotate_blk(unsigned long long cv[8], const uint32_t rot_value)
{
...
static inline void  xor_with_const(unsigned long long cv_l[8], const unsigned long long* const_v)
{
	cv_l[0] ^= const_v[0];
	cv_l[1] ^= const_v[1];
	cv_l[2] ^= const_v[2];
...
static inline void  rotate_msg_gamma(unsigned long long cv_r[8])
{
...
static inline void  word_perm(unsigned long long cv_l[8], unsigned long long cv_r[8])
{
	unsigned long long temp;
	temp = cv_l[0];
	cv_l[0] = cv_l[6];
...
static inline void mix(unsigned long long cv_l[8], unsigned long long cv_r[8], const unsigned long long const_v[8], const int32_t rot_alpha, const int32_t rot_beta)
{
	add_blk(cv_l, cv_r);
...
static inline void compress(struct LSH512_Context* ctx, const unsigned long long pdMsgBlk[32])
{
	uint32_t i;
	struct LSH512_internal i_state[1];

	const unsigned long long* const_v = 0;
	unsigned long long*cv_l = ctx->cv_l;
	unsigned long long*cv_r = ctx->cv_r;

	load_msg_blk(i_state, pdMsgBlk);
	msg_add_even(cv_l, cv_r, i_state);
	load_sc(&const_v, 0);
	mix(cv_l, cv_r, const_v, 23, 59);
	word_perm(cv_l, cv_r);
	msg_add_odd(cv_l, cv_r, i_state);
	load_sc(&const_v, 8);
	mix(cv_l, cv_r, const_v, 7, 3);
	word_perm(cv_l, cv_r);
	for (i = 1; i < 28 / 2; i++)
	{
...
static inline void get_hash(struct LSH512_Context* ctx, uint8_t* pbHashVal, const uint32_t algtype)
{
	uint32_t hash_val_byte_len = LSH_GET_HASHBYTE(algtype);
	uint32_t hash_val_bit_len = LSH_GET_SMALL_HASHBIT(algtype);
	memcpy(pbHashVal, ctx->cv_l, sizeof(uint8_t)* hash_val_byte_len);
	if (hash_val_bit_len){
		pbHashVal[hash_val_byte_len-1] &= (((uint8_t)0xff) << hash_val_bit_len);
	}
}
...
uint32_t lsh512_create(void** hdl)
{
	*hdl = (struct LSH512_Context*)calloc(1, sizeof(struct LSH512_Context));
	return 0;
}

uint32_t lsh512_destroy(void** hdl)
{
	if (*hdl )
	{
		free(*hdl);
...
uint32_t lsh512_init(void* hdl, const uint32_t algtype)
{
  struct LSH512_Context* ctx = (struct LSH512_Context*)hdl;
	uint32_t i;
	unsigned long long* cv_l;
	unsigned long long* cv_r;
	...
	if (LSH_GET_HASHBYTE(algtype) > 64 || LSH_GET_HASHBYTE(algtype) == 0)
	{
		return LSH_ERR_INVALID_ALGTYPE;
	}
	...
	memset(cv_l, 0, 8* sizeof(unsigned long long));
	memset(cv_r, 0, 8* sizeof(unsigned long long));
	cv_l[0] = 64;
...
uint32_t lsh512_update(void* hdl, const uint8_t* data, unsigned long long databitlen)
{
  struct LSH512_Context* ctx = (struct LSH512_Context*)hdl;
	unsigned long long databytelen = databitlen >> 3;
	uint32_t pos2 = databitlen & 0x7;
	uint32_t remain_msg_byte;
	uint32_t remain_msg_bit;
	...
	if (ctx->algtype == 0 || LSH_GET_HASHBYTE(ctx->algtype) > 64)
	{
	...
	if (databytelen + remain_msg_byte < 256)
	{
		memcpy(ctx->last_block + remain_msg_byte, data, databytelen);
		ctx->remain_databitlen += (uint32_t)databitlen;
		remain_msg_byte += (uint32_t)databytelen;
		if (pos2){
	...
	{
		uint32_t more_byte = 256 - remain_msg_byte;
		memcpy(ctx->last_block + remain_msg_byte, data, more_byte);
		compress(ctx, (unsigned long long*)ctx->last_block);
		data += more_byte;
	...
	while (databytelen >= 256)
	{
		compress(ctx, (unsigned long long*)data);
		data += 256;
		databytelen -= 256;
	}
	...
	if (databytelen > 0)
	{
		...
		ctx->remain_databitlen = (uint32_t)(databytelen << 3);
	}
...
uint32_t lsh512_final(void* hdl, uint8_t* hashval)
{
  struct LSH512_Context* ctx = (struct LSH512_Context*)hdl;
	uint32_t remain_msg_byte;
	uint32_t remain_msg_bit;
	...
	memset(ctx->last_block + remain_msg_byte + 1, 0, 256 - remain_msg_byte - 1);
	compress(ctx, (unsigned long long*)ctx->last_block);
	fin(ctx);
...
uint32_t lsh512_digest(const uint32_t algtype, const uint8_t* data, unsigned long long databitlen, uint8_t* hashval)
{
	uint32_t result;
