...
{
	uint32_t submsg_e_l[16 / 2];	/* even left sub-message */
	uint32_t submsg_e_r[16 / 2];	/* even right sub-message */
	uint32_t submsg_o_l[16 / 2];	/* odd left sub-message */
	uint32_t submsg_o_r[16 / 2];	/* odd right sub-message */
};


/********        lsh 256        ********/
static const uint32_t g_IV224[16] = 
{
...
static const uint32_t g_IV256[16] = 
{
...
static const uint32_t g_gamma256[8] = { 0, 8, 16, 24, 24, 16, 8, 0 };

static const uint32_t g_StepConstants[8 * 26] = 
{
...
static inline void load_msg_blk(struct LSH256_internal * i_state, const uint32_t * msgblk)
{
...
static inline void load_sc(const uint32_t** p_const_v, uint32_t i)
{
	*p_const_v = &g_StepConstants[i];
}

static inline void msg_add_even(uint32_t* cv_l, uint32_t* cv_r, struct LSH256_internal * i_state)
{
...
static inline void msg_add_odd(uint32_t* cv_l, uint32_t* cv_r, struct LSH256_internal * i_state)
{
...
static inline void  add_blk(uint32_t* cv_l, const uint32_t* cv_r)
{
	cv_l[0] += cv_r[0];
	cv_l[1] += cv_r[1];
	cv_l[2] += cv_r[2];
...
static inline void  rotate_blk(uint32_t cv[8], const int32_t rot_value)
{
...
static inline void  xor_with_const(uint32_t* cv_l, const uint32_t* const_v)
{
	cv_l[0] ^= const_v[0];
	cv_l[1] ^= const_v[1];
	cv_l[2] ^= const_v[2];
...
static inline void  rotate_msg_gamma(uint32_t* cv_r)
{
...
static inline void  word_perm(uint32_t* cv_l, uint32_t* cv_r)
{
	uint32_t temp;
	temp = cv_l[0];
	cv_l[0] = cv_l[6];
...
static inline void mix(uint32_t* cv_l, uint32_t* cv_r, const uint32_t* const_v, const int32_t rot_alpha, const int32_t rot_beta)
{
	add_blk(cv_l, cv_r);
...
static inline void compress(struct LSH256_Context* ctx, const uint32_t pdMsgBlk[32])
{
	uint32_t i;
	struct LSH256_internal i_state[1];

	const uint32_t* const_v = 0;
	uint32_t* cv_l = ctx->cv_l;
	uint32_t* cv_r = ctx->cv_r;
...
static inline void get_hash(struct LSH256_Context * ctx, uint8_t * pbHashVal, const uint32_t algtype)
{
...
int32_t lsh256_create(void** hdl)
{
	*hdl = (struct LSH256_Context*)calloc(1, sizeof(struct LSH256_Context));
	return 0;
}

int32_t lsh256_destroy(void** hdl)
{
	if ( *hdl )
	{
		free(*hdl);
...
int32_t lsh256_init (void* hdl, const uint32_t algtype)
{
	struct LSH256_Context* ctx = (struct LSH256_Context*)hdl;
	uint32_t i;
	uint32_t* cv_l;
	uint32_t* cv_r;
	...
	if (LSH_GET_HASHBYTE(algtype) > 32 || LSH_GET_HASHBYTE(algtype) == 0)
	{
		return 0xE0000001;
	}
	...
	memset(ctx->cv_l, 0, 8 * sizeof(uint32_t));
	memset(ctx->cv_r, 0, 8 * sizeof(uint32_t));
	ctx->cv_l[0] = 32;
...
int32_t lsh256_update(void* hdl, const uint8_t * data, uint32_t databitlen)
{
	struct LSH256_Context* ctx = (struct LSH256_Context*)hdl;
	...
	uint32_t remain_msg_byte = 0;
	uint32_t remain_msg_bit = 0;
	...
	if (databitlen == 0)
	{
 	...
 	if (ctx->algtype == 0 || LSH_GET_HASHBYTE(ctx->algtype) > 32)
 	{
 		return 0xE0000002;
 	}
	...
	{
		uint32_t more_byte = 128 - remain_msg_byte;
		memcpy(ctx->last_block + remain_msg_byte, data, more_byte);
		compress(ctx, (uint32_t*)ctx->last_block);
		data += more_byte;
	...
	while (databytelen >= 128)
	{
		compress(ctx, (uint32_t*)data);
		data += 128;
		databytelen -= 128;
	}
...
int32_t lsh256_final(void* hdl, uint8_t* hashval)
{
	struct LSH256_Context* ctx = (struct LSH256_Context*)hdl;
	uint32_t remain_msg_byte = 0;
	uint32_t remain_msg_bit = 0;
	...
	memset(ctx->last_block + remain_msg_byte + 1, 0, 128 - remain_msg_byte - 1);
...
int32_t lsh256_digest(const uint32_t algtype, const uint8_t * data, uint32_t databitlen, uint8_t * hashval)
{
	int32_t result;
