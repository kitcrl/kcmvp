...
void RotXOR (const uint8_t *s, int32_t n, uint8_t *t)
{
	int32_t i, q;
...
int32_t ekey_setup(const uint8_t *w0, uint8_t *e, int32_t keyBits) {
	int32_t  i, R=(keyBits+256)/32, q;
...
int32_t dkey_setup(const uint8_t *w0, uint8_t *d, int32_t keyBits) {
	int32_t  i, j, R;
	uint8_t t[16];
  
	R=ekey_setup(w0, d, keyBits);
	for (j = 0; j < 16; j++){
		t[j] = d[j];
		d[j] = d[16*R + j];
		d[16*R + j] = t[j];
...
void aria_crypt(const uint8_t *p, int32_t R, const uint8_t *e, uint8_t *c)
{
	int32_t i, j;
	uint8_t t[16];
